...
string horsePath = "./modelSequence/horses/horses_";
...
GLuint textureLoad() {
	...
	unsigned char* data = stbi_load("./texture/grass.png", &w, &h, &channels, 0);
...
//create horses vector
vector<objLoader> horses;
...
void display()
{
	...
	horses[indexOfHorse].draw(shadeType);
	glPopMatrix();
	glPopAttrib();
...
void myInit(bool shadeType) {
	//load horse
	for (int i = 0; i < 117; i++) {
		horses.push_back(objLoader(horsePath + to_string(i) + ".obj"));
		cout << "Loading horse: " << i << endl;
	}
